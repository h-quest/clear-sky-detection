...
class ClearSkyShapExplainer:
    ...
    def plot_multiple_forces_from_cluster(self, X: pd.DataFrame, y: pd.Series, cluster_results: dict, 
        ...
        original_indices = cluster_results['original_indices']
        ...
        print(f"Total samples in cluster: {len(cluster_indices)}")
        
        # Sample from the cluster
        if len(cluster_indices) <= n_samples:
            selected_cluster_indices = cluster_indices
            print(f"Using all {len(selected_cluster_indices)} samples from cluster {cluster_id}")
        else:
            selected_cluster_indices = np.random.choice(cluster_indices, size=n_samples, replace=False)
            print(f"Selected {n_samples} random samples from cluster {cluster_id} (total: {len(cluster_indices)} samples)")
        
        # Map cluster indices back to original dataset indices
        selected_original_indices = [original_indices[i] for i in selected_cluster_indices]
        
        # Print sample information
        print("\nSelected samples:")
        for cluster_idx, orig_idx in zip(selected_cluster_indices, selected_original_indices):
            print(f"\nSample (Cluster Index: {cluster_idx}, Original Index: {orig_idx}):")
            print(f"Timestamp: {X.index[orig_idx]}")
            ...
            for feature in available_features:
                print(f"{feature}: {X.iloc[orig_idx][feature]:.4f}")
        ...
        for orig_idx in selected_original_indices:
            print(f"\nSample {orig_idx}:")
            print(f"Prediction probability: {y_pred_proba[orig_idx]:.3f}")
            print(f"True label: {y[orig_idx]}")
            misclass_type = "False Positive" if y[orig_idx] == 0 and y_pred_proba[orig_idx] >= 0.5 else "False Negative"
            print(f"Classification: {misclass_type}")
            
        # Plot the selected samples using original indices
        return self.plot_multiple_forces(X, selected_original_indices, y)
    ...
    def explain_prediction(self, X: pd.DataFrame, sample_idx: int = 0, 
        ...
        # Get prediction
        prediction = self.model.predict_proba(X_processed)[0, 1]
    ...
    def cluster_misclassifications(self, X: pd.DataFrame, y: Union[pd.Series, np.ndarray], 
        ...
        #     print(f"Warning: Dataset contains datetime columns {list(datetime_cols)} which will be excluded from analysis")
        #     X_for_pred = X_for_pred.drop(columns=datetime_cols)
        ...
        # nan_cols = X_processed.columns[X_processed.isna().any()].tolist()
        # if nan_cols:
        #     print(f"Warning: Found NaN values in columns after processing: {nan_cols}")
        ...
        # Print some example false positives
        fp_indices = np.where(false_positives)[0]
        ...
        if misclassification_type in ['false_positives', 'both'] and false_positives.sum() > 0:
            print(f"\n===== CLUSTERING FALSE POSITIVES =====")
            print(f"Found {false_positives.sum()} false positives")
            
            # Important: Store the original indices of false positives
            fp_original_indices = np.where(false_positives)[0]
            print("\nDEBUG - Original Indices:")
            print(f"First 5 fp_original_indices: {fp_original_indices[:5]}")
    
            # Get false positive samples
            X_fp = X_processed[false_positives].copy()
            
            print("\n=== X_fp Initial Check ===")
            print(f"X_fp shape: {X_fp.shape}")
            print("X_fp NaN check:")
            print(X_fp.isna().sum()[X_fp.isna().sum() > 0])
            
            # Prepare data for clustering
            if use_shap_values:
                # Compute SHAP values
                print("Computing SHAP values for false positives...")
                shap_values_fp = self.explainer(X_fp)
                
                # Use SHAP values for clustering
                cluster_data = pd.DataFrame(
                    shap_values_fp.values,
                    columns=self.feature_names,
                    index=X_fp.index
                )
                print("\n=== Cluster Data from SHAP values ===")
                ...
                # Use raw features
                # X_fp_processed, _ = self.model._prepare_features(X_fp, is_training=False)
                cluster_data = X_fp
            ...
            if n_clusters is None:
                ...
                silhouette_scores = []
                range_n_clusters = range(2, min(10, len(X_fp) // 10))
            ...
            # Analyze clusters
            cluster_stats = self._analyze_clusters(X_fp, cluster_labels, n_clusters, X, y)
            
            # Visualize clusters
            fig = self._visualize_clusters(X_fp, cluster_labels, n_clusters, 
                                        cluster_stats, 'False Positives')
            
            # Plot multiple forces for the first 5 false positives
            if len(fp_original_indices) > 0:
                print("\nDEBUG - Before Plotting Verification:")
                for idx in fp_original_indices[:5]:
                    print(f"\nVerifying index {idx}:")
                    print(f"Original probability: {y_pred_proba[idx]:.3f}")
                    print(f"Original actual label: {y[idx]}")
                    print(f"Original predicted label: {y_pred[idx]}")
                    print(f"Is in false positives mask: {false_positives[idx]}")
                
                print(f"\nPlotting SHAP force plots for the first {min(5, len(fp_original_indices))} false positives...")
                self.plot_multiple_forces(X, fp_original_indices[:5], y)
            ...
            results['false_positives'] = {
                ...
                'figure': fig,
                'original_indices': fp_original_indices
            }
        
        # Process false negatives
        if misclassification_type in ['false_negatives', 'both'] and false_negatives.sum() > 0:
            print(f"\n===== CLUSTERING FALSE NEGATIVES =====")
            print(f"Found {false_negatives.sum()} false negatives")
            
            # Get false negative samples
            X_fn = X_processed[false_negatives].copy()
            
            # Prepare data for clustering
            if use_shap_values:
                # Compute SHAP values
                print("Computing SHAP values for false negatives...")
                shap_values_fn = self.explainer(X_fn)
                
                # Use SHAP values for clustering
                cluster_data = pd.DataFrame(
                    shap_values_fn.values,
                    columns=self.feature_names,
                    index=X_fn.index
                )
            else:
                # Use raw features
                # X_fn_processed, _ = self.model._prepare_features(X_fn, is_training=False)
                cluster_data = X_fn
            ...
            if n_clusters is None:
                print("Finding optimal number of clusters...")
                silhouette_scores = []
                range_n_clusters = range(2, min(10, len(X_fn) // 10))
                for n in range_n_clusters:
                    kmeans = KMeans(n_clusters=n, random_state=random_state, n_init=10)
                    cluster_labels = kmeans.fit_predict(scaled_data)
                    score = silhouette_score(scaled_data, cluster_labels)
            ...
            # Analyze clusters
            cluster_stats = self._analyze_clusters(X_fn, cluster_labels, n_clusters, X, y)
            
            # Visualize clusters
            fig = self._visualize_clusters(X_fn, cluster_labels, n_clusters, 
                                        cluster_stats, 'False Negatives')
    ...
    def _visualize_clusters(self, X_clustered, cluster_labels, n_clusters, cluster_stats, title_prefix='Clusters'):
        """
        ...
        # Try to get enough features for PCA - IMPORTANT: Filter for numeric features only
        pca_features = X_clustered.select_dtypes(include=np.number).columns.tolist()
        # Remove 'cluster' column if present
        if 'cluster' in pca_features:
            pca_features.remove('cluster')
        
        if len(pca_features) >= 2:
            # Perform PCA
            pca = PCA(n_components=2)
            # Make sure we use only numeric features
            pca_result = pca.fit_transform(X_clustered[pca_features])
            
            # Plot PCA results
            scatter_pca = ax2.scatter(
                pca_result[:, 0], 
                pca_result[:, 1],
                c=cluster_labels, 
                cmap='viridis', 
            ...
            for cluster_id in range(n_clusters):
                cluster_mask = cluster_labels == cluster_id
                centroid_x = pca_result[cluster_mask, 0].mean()
                centroid_y = pca_result[cluster_mask, 1].mean()
            ...
            # Add labels
            ax2.set_title(f"{title_prefix}: PCA Visualization", fontsize=14)
            ax2.set_xlabel(f"Principal Component 1 ({pca.explained_variance_ratio_[0]:.2%})", fontsize=12)
            ax2.set_ylabel(f"Principal Component 2 ({pca.explained_variance_ratio_[1]:.2%})", fontsize=12)
            ax2.grid(alpha=0.3)
        else:
            ax2.text(0.5, 0.5, "Not enough numeric features for PCA", 
                    ha='center', va='center', fontsize=14)
        
        # Plot 3-5: Cluster profiles for each cluster (parallel coordinates)
        ...
        # Create ax objects for each cluster
        ...
        bars = ax6.bar(range(n_clusters), sizes)
        ...
        # Add title and labels
        ax6.set_title(f"{title_prefix}: Cluster Sizes", fontsize=14)
        ax6.set_xlabel("Cluster", fontsize=12)
        ax6.set_ylabel("Number of Samples", fontsize=12)
        ax6.grid(axis='y', alpha=0.3)
        
        # Plot 7: Feature importance by cluster (heatmap)
        ...
        if numeric_subset and n_clusters > 0:
            ...
            if heatmap_data and any(row for row in heatmap_data):
                ...
                # Customize heatmap
                ax7.set_title(f"{title_prefix}: Feature Deviation from Mean by Cluster (%)", fontsize=14)
                ax7.set_xlabel("Feature", fontsize=12)
                ax7.set_ylabel("Cluster", fontsize=12)
                plt.setp(ax7.get_xticklabels(), rotation=45, ha='right')
            else:
                ax7.text(0.5, 0.5, "Insufficient data for heatmap", 
                        ha='center', va='center', fontsize=14)
        else:
            ax7.text(0.5, 0.5, "No numeric features available for heatmap", 
                    ha='center', va='center', fontsize=14)
        
        # Add overall title
        fig.suptitle(f"{title_prefix} Cluster Analysis (n={len(X_clustered)})", fontsize=16)
